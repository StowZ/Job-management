	...
	class Smk_Sidebar_Generator extends Smk_Sidebar_Generator_Abstract {
		...
		public function pageOpen($echo = true){
			...
			$html .= '<h2>'. $this->pluginSettings( 'name' ) .'
				<span class="add-new-h2 add-new-sidebar" data-sidebars-prefix="'. $this->prefix() .'">'. __('Add new', 'noo') .'</span>
			</h2>';
			$html .= '<div class="smk-sidebars-grid">';
			$html .= '<h3>
					'. __('Sidebars', 'noo') .'
					<span class="tip dashicons-before dashicons-editor-help" title="'. __('All available sidebars.', 'noo') .'"></span>
				</h3>';
		...
		public function aSingleListItem($sidebar_data, $settings = false){
			...
			if( !empty($sidebar_data) ) : 
				$the_sidebar = $this->sidebarAccordion('open', $sidebar_data, $settings, false);
					...
					$the_sidebar .= ' <button class="condition-add button"'. $disbled_conditions_btn .' data-name="'. $name .'" data-sidebar-id="'. $sidebar_data['id'] .'">'. __('Add condition', 'noo') .'</button>';
				...
				<li id="'. $sidebar_data['id'] .'" class="control-section accordion-section'. $class .'">
					<h3 class="accordion-section-title hndle">
						...
						<div class="moderate-sidebar">
							<span class="smk-delete-sidebar">'. __('Delete', 'noo') .'</span>
							<span class="smk-restore-sidebar">'. __('Restore', 'noo') .'</span>
						</div>
					</h3>
		...
		public function aSingleCondition($name, $sidebar_data, $index = 0, $condition_if = 'all'){
			$the_sidebar = '<div class="condition-parent sbg-clearfix">';
				...
				$the_sidebar .= '</div>';
			$the_sidebar .= ' <span class="condition-remove" title="'. __('Remove condition', 'noo') .'"> <i class="dashicons dashicons-no-alt"></i> </span>';
			$the_sidebar .= '</div>';
			return $the_sidebar;
		}
		...
		public function fieldDescription($name, $sidebar_data){
			return '<div class="smk-sidebar-row smk-sidebar-grid-8">
				<label>'. __('Description:', 'noo') .'</label>'. 
				$this->html->input(
					'', // ID
					$name. '[description]', 
		...
		public function fieldToReplace($name, $sidebar_data){
			...
			return '<div class="smk-sidebar-row smk-sidebar-grid-4">
				<label>'. __('Sidebars to replace:', 'noo') .'</label>'. 
				$this->html->select(
					'', // ID
					$name. '[replace][]', 
					$replace,
		...
		public function fieldConditionMain($name, $sidebar_data, $index = 0){
			...
			$options = array( 'none' => __('None', 'noo') );
			...
			return '<span class="condition-label">'. __('Replace if', 'noo') .' </span>'.
				$this->html->select(
					'', // ID
		...
		public function fieldConditionEqualTo($name, $sidebar_data, $index = 0, $type){
			...
			return '<span class="condition-label">'. __('and is equal to', 'noo') .'</span>' . 
				$this->html->select(
					'', // ID
		...
		public function allRemovedSidebarsList($echo = true){
			$list = '<div class="smk-sidebars-grid removed-sidebars">
				<h3>
					'. __('Removed', 'noo') .'
					<span class="tip dashicons-before dashicons-editor-help" title="'. __('These sidebars will be removed on the next page refresh.', 'noo') .'"></span>
				</h3>
		...
		public function sidebarListTemplate($echo = true){
			$sidebar_data = array( 
				'name'        => sprintf( __('New sidebar %s', 'noo'), '__index__' ),
				'id'          => '__id__',
				'description' => '',
			);
